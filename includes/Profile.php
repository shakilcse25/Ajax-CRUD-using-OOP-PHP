...
class Profile extends Database
{
    ...
    {
        $profile = $this->getRow('id',$id);

        $path = getcwd().'/uploads/'.$profile['photo'];
        if (!empty($profile['photo']) && file_exists($path)) {
          unlink($path);
        }
    ...
    public function searchProfile($searchText, $start = 0, $limit = 4)
    {
        $sql = "SELECT * FROM {$this->tableName} WHERE fname LIKE :search OR lname LIKE :search ORDER BY id DESC LIMIT {$start},{$limit}";
        $stmt = $this->conn->prepare($sql);
